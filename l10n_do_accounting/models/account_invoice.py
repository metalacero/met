...
class AccountInvoice(models.Model):
        ...
        for inv in self:
            if inv.is_l10n_do_fiscal_invoice and inv.is_invoice():
                ...
                if inv.amount_total == 0:
                    raise UserError(
                        _(
                            "You cannot validate an invoice whose "
                            "total amount is equal to 0"
                        )
                    )
                ...
                if inv.move_type == "out_invoice":
                    # Set default company stamp for customer invoices
                    stamp_attachment = self.env.ref(
                        "l10n_do_accounting.default_invoice_stamp",
                        raise_if_not_found=False,
                    )
                    if stamp_attachment:
                        inv.company_stamp = stamp_attachment.datas
                ...
                if inv.fiscal_type_id.requires_document and not inv.partner_id.vat:
                    ...
                    if (
                        ...
                        raise UserError(
                            _(
                                "if the invoice amount is greater than "
                                "RD$250,000.00 "
                                "the customer should have RNC or Céd"
                                "for make invoice"
                            )
                        )
    ...
    def button_cancel(self):
        if (
            ...
            and self.journal_id.l10n_do_fiscal_journal
            and self.env.context.get("skip_cancel_wizard", False) == False
        ):
    ...
    def create(self, vals_list):
        ...
        for fiscal_invoice in fiscal_invoices:
            ...
            fiscal_invoice.filtered(lambda i: i.move_type not in ["in_refund"]).write(
                {"ref": "", "payment_reference": fiscal_invoice.ref}
            )
