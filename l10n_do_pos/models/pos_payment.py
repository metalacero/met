        ...
        if pos_payment_cash:
            ...
            if cash_payment_values['amount'] > 0:
                ...
                try:
                    ...
                    # Manejar error cuando account_check_printing intenta referenciar un registro que no existe
                    if 'account_check_printing.account_payment_method_check' in str(e):
                        # El módulo account_check_printing está instalado pero el registro XML no existe
                        # Intentar publicar el pago sin pasar por account_check_printing usando el método padre
                        account_payment_cash = account_payment_cash.with_context(skip_check_printing=True)
                        # Llamar directamente al método padre de action_post para evitar account_check_printing
                        super(self.env['account.payment'].__class__, account_payment_cash).action_post()
                    else:
                        raise
