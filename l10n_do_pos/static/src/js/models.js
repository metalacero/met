odoo.define("l10n_do_pos.models", function (require) {
    ...
    const L10nDoPosPosGlobalState = (PosGlobalState) =>
        class extends PosGlobalState {
            ...
            get_fiscal_type_by_prefix(prefix) {
                ...
                if (res_fiscal_type) return res_fiscal_type;

                Gui.showPopup("ErrorPopup", {
                    title: _t("Fiscal type not found"),
                    body: _.str.sprintf(
                        _t("This fiscal type not exist. (%s)"),
                        prefix,
                    ),
                });
            ...
            get_credit_note_payment_method() {
                ...
                this.env.pos.payment_methods.forEach(function (payment_method) {
    ...
    const L10nDoPosOrder = (Order) =>
        class extends Order {
            /**
             * Override add_orderline to merge lines that originate from the same
             * sale.order.line.
             *
             * When a sale order is settled from a custom (non-groupable) UoM, the
             * base pos_sale code splits lines into qty=1 units. This override
             * detects those split lines (same sale_order_line_id) and accumulates
             * the quantity on the first one instead of creating duplicate rows.
             */
            add_orderline(line) {
                if (line.sale_order_line_id) {
                    const existing = this.get_orderlines().find(
                        (l) =>
                            l.sale_order_line_id &&
                            l.sale_order_line_id.id ===
                                line.sale_order_line_id.id,
                    );
                    if (existing) {
                        existing.set_quantity(
                            existing.get_quantity() + line.get_quantity(),
                        );
                        this.select_orderline(existing);
                        return;
                    }
                }
            ...
            set_fiscal_type(fiscal_type) {
                ...
                if (fiscal_type && fiscal_type.fiscal_position_id) {
                    this.set_fiscal_position(
                        _.find(this.pos.fiscal_positions, function (fp) {
            ...
            init_from_JSON(json) {
                ...
                if (this.pos.config.l10n_do_fiscal_journal) {
                    ...
                    if (json.fiscal_type_id)
                        this.set_fiscal_type(
                            this.pos.get_fiscal_type_by_id(json.fiscal_type_id),
                        );
    ...
    const L10nDoPayment = (Payment) =>
        class extends Payment {
            ...
            constructor(obj, options) {
                ...
                this.credit_note_partner_id =
                    this.credit_note_partner_id || false;
            }
            //@override
            export_as_JSON() {
                const json = super.export_as_JSON(...arguments);
    ...
    const L10nDoPosOrderLine = (Orderline) =>
        class extends Orderline {
            /**
             * Override to allow settling a sale order via POS even when the
             * warehouse delivery has already been validated before payment.
             *
             * The base pos_sale implementation uses:
             *   product_uom_qty - Math.max(qty_delivered, qty_invoiced)
             *
             * When qty_delivered equals product_uom_qty (delivery done first),
             * that formula gives 0 and the line appears empty in the POS.
             *
             * Using only qty_invoiced lets the cashier settle the full remaining
             * balance regardless of delivery state.
             */
            ...
            get_itbis() {
                ...
                for (const tax_id in tax_details) {
                    if (
                        this.pos.taxes_by_id[tax_id].tax_group_id[1] === "ITBIS"
                    ) {
