...
class SaleOrderLine(models.Model):
    ...
    def read_converted(self):
        ...
        _logger.info(
            "read_converted base returned %s results (esperábamos %s líneas)",
            len(results),
            len(self),
        )
        ...
        for idx, result in enumerate(results):
            ...
            if isinstance(result_id, (list, tuple)) and len(result_id) > 1:
                id_str = (
                    f"{result_id[0]} (con {len(result_id) - 1} elementos adicionales)"
                )
            ...
            _logger.info(
                ...
                result.get("price_unit"),
                result.get("qty_to_invoice", 0),
                result.get("product_uom_qty", 0),
                result.get("qty_delivered", 0),
                result.get("qty_invoiced", 0),
            )
            ...
            if product_id is None:
                ...
                if result_id and result_id in self.ids:
                    ...
                    if sale_line.exists() and sale_line.product_id:
                        product_id = sale_line.product_id.id
                        _logger.info(
                            "Producto obtenido desde línea de venta %s: %s",
                            result_id,
                            product_id,
                        )
        ...
        for sale_line in self:
            ...
            if product_id not in sale_lines_by_product_id:
                ...
                _logger.info(
                    ...
                    sale_line.id,
                    product_id,
                    sale_lines_qty_by_product_id[product_id],
                )
        ...
        for product_id, grouped_results in results_by_product_id.items():
            ...
            if not isinstance(grouped_results, list):
                _logger.warning(
                    "grouped_results no es una lista para producto %s: %s",
                    product_id,
            ...
            if product_id in sale_lines_by_product_id:
                ...
                if not isinstance(first_result, dict):
                    _logger.warning(
                        "Primer resultado no es un diccionario para producto %s",
                        product_id,
                    )
                    continue
            ...
            if matching_sale_line and matching_sale_line.exists():
                ...
                total_qty_from_sale_lines = sale_lines_qty_by_product_id.get(
                    product_id, matching_sale_line.product_uom_qty
                )
                ...
                if len(grouped_results) == 1:
                    ...
                    _logger.info(
                        ...
                        len(grouped_results),
                        product_id,
                        matching_sale_line.id,
                        matching_sale_line.product_id.display_name
                        if matching_sale_line.product_id
                        else "N/A",
                    )
                    ...
                    for i, r in enumerate(grouped_results):
                        ...
                        _logger.info(
                            ...
                            i + 1,
                            r.get("qty_to_invoice", 0),
                            r.get("product_uom_qty", 0),
                            r.get("qty_delivered", 0),
                            r.get("price_unit", 0),
                            result_id,
                        )
                    consolidated_result = self._consolidate_split_lines(
                        matching_sale_line, grouped_results
                    )
                ...
                if len(grouped_results) > 1:
                    ...
                    consolidated_result = self._consolidate_split_lines_by_results(
                        grouped_results
                    )
                    consolidated_results.append(consolidated_result)
                else:
                    ...
                    if isinstance(grouped_results, list):
                        ...
                        _logger.warning(
                            "grouped_results no es una lista: %s", type(grouped_results)
                        )
                        if grouped_results:
        ...
        for result in consolidated_results:
            if result is not None and isinstance(result, dict):
                ...
                _logger.warning(
                    "Resultado inválido omitido: %s (tipo: %s)", result, type(result)
                )
    ...
    def _process_single_result(self, sale_line, result):
        """
        Procesa un único resultado del método base read_converted para asegurar
        que las cantidades sean correctas cuando el picking ya está en estado 'done'.

        Cuando qty_to_invoice == 0 (porque la entrega ya fue facturada o la cantidad
        entregada iguala la pedida), el POS necesita la cantidad original para poder
        facturar correctamente.

        :param sale_line: sale.order.line — la línea de venta que corresponde al resultado
        :param result: dict — resultado devuelto por el método base read_converted
        :return: dict — resultado ajustado con las cantidades correctas
        """
        if not isinstance(result, dict):
            _logger.warning(
                "_process_single_result: result no es dict: %s", type(result)
            )
            return result

        qty_to_invoice = result.get("qty_to_invoice", 0)
        product_uom_qty = result.get("product_uom_qty", 0)

        # Si qty_to_invoice es 0 pero la línea tiene cantidad, usar product_uom_qty
        if qty_to_invoice == 0 and product_uom_qty > 0:
            _logger.info(
                "_process_single_result: qty_to_invoice=0 para línea %s (producto %s), "
                "usando product_uom_qty=%s",
                sale_line.id,
                sale_line.product_id.display_name if sale_line.product_id else "N/A",
                product_uom_qty,
            )
            result["qty_to_invoice"] = product_uom_qty

        # Garantizar que el ID del resultado sea siempre el de la línea de venta
    ...
    def _consolidate_split_lines(self, sale_line, grouped_results):
        """
        Consolida múltiples resultados del método base que corresponden al mismo
        producto (ocurre cuando Odoo divide una línea por entregas parciales) en
        un único resultado con las cantidades sumadas.

        :param sale_line: sale.order.line — la línea de venta "maestra" del grupo
        :param grouped_results: list[dict] — resultados a consolidar
        :return: dict — resultado consolidado
        """
        if not grouped_results:
            _logger.warning(
                "_consolidate_split_lines: grouped_results vacío para línea %s",
                sale_line.id,
            )
            return {}

        # Usar el primer resultado como base
        base_result = dict(grouped_results[0])

        total_qty_to_invoice = 0.0
        ...
        for r in grouped_results:
            if not isinstance(r, dict):
                _logger.warning(
                    "_consolidate_split_lines: resultado no es dict: %s", type(r)
                )
                continue
            total_qty_to_invoice += r.get("qty_to_invoice", 0) or 0
            total_product_uom_qty += r.get("product_uom_qty", 0) or 0
            total_qty_delivered += r.get("qty_delivered", 0) or 0
            total_qty_invoiced += r.get("qty_invoiced", 0) or 0

        # Si después de sumar la cantidad a facturar sigue en 0, usar la cantidad total pedida
        if total_qty_to_invoice == 0 and total_product_uom_qty > 0:
            _logger.info(
                "_consolidate_split_lines: qty_to_invoice=0 tras consolidar línea %s, "
                "usando product_uom_qty=%s",
                sale_line.id,
        ...
        base_result["qty_to_invoice"] = total_qty_to_invoice
        base_result["product_uom_qty"] = total_product_uom_qty
        base_result["qty_delivered"] = total_qty_delivered
        base_result["qty_invoiced"] = total_qty_invoiced
        base_result["id"] = sale_line.id

        _logger.info(
            "_consolidate_split_lines: línea %s consolidada → qty_to_invoice=%s, product_uom_qty=%s",
            sale_line.id,
    ...
    def _consolidate_split_lines_by_results(self, grouped_results):
        """
        Consolida múltiples resultados cuando no existe una línea de venta maestra
        disponible (fallback). Usa el primer resultado como base y suma las cantidades
        del resto.
        ...
        if not grouped_results:
            _logger.warning(
                "_consolidate_split_lines_by_results: grouped_results vacío"
            )
            return {}
        ...
        for r in grouped_results:
            if not isinstance(r, dict):
                _logger.warning(
                    "_consolidate_split_lines_by_results: resultado no es dict: %s",
                    type(r),
                )
                continue
            total_qty_to_invoice += r.get("qty_to_invoice", 0) or 0
        ...
        # Fallback: si qty_to_invoice sigue en 0, usar el total pedido
        if total_qty_to_invoice == 0 and total_product_uom_qty > 0:
            total_qty_to_invoice = total_product_uom_qty
        ...
        _logger.info(
            "_consolidate_split_lines_by_results: consolidado sin línea maestra → "
            "qty_to_invoice=%s, product_uom_qty=%s",
            total_qty_to_invoice,
            total_product_uom_qty,
        )
