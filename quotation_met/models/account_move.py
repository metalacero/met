            ...
            if "invoice_type" in fields_list:
                try:
                    ...
                    if pos_session:
                        ...
                        _logger.info(
                            "Invoice created from POS: setting invoice_type=contado"
                        )
                except Exception as e:
                    _logger.debug(
                        "Error verifying POS session in default_get: %s", str(e)
                    )
        ...
        if is_from_pos:
            for vals in vals_list:
                ...
                if move_type in (
                    ...
                    _logger.info(
                        "Factura creada desde POS: estableciendo invoice_type=contado"
                    )
        ...
        for invoice in invoices:
            if invoice.move_type in (
                ...
                pos_order = (
                    self.env["pos.order"]
                    .sudo()
                    .search([("account_move", "=", invoice.id)], limit=1)
                )

                if pos_order and invoice.invoice_type == "credito":
                    invoice.invoice_type = "contado"
                    _logger.info(
                        "Factura %s relacionada con POS order: estableciendo invoice_type=contado",
                        invoice.name,
                    )
